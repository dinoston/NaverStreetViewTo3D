...
set "PATH=%CONDA_ENV%\bin;%CONDA_ENV%\Library\bin;%CONDA_ENV%\Scripts;%PATH%"
...
"%CONDA_ENV%\python.exe" -m pip install --no-build-isolation ^
  "%PROJECT_ROOT%\external\gaussian-splatting\submodules\diff-gaussian-rasterization" ^
  "%PROJECT_ROOT%\external\gaussian-splatting\submodules\simple-knn" ^
  "%PROJECT_ROOT%\external\gaussian-splatting\submodules\fused-ssim"
exit /b %errorlevel%
