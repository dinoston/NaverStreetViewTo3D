param(
    [ValidateSet("doctor", "preprocess", "align", "train", "mesh", "fast", "panorama-fast", "splat", "splat-mesh", "all")]
    [string]$Stage = "all",
    [switch]$Force
)
...
if ($Force -and $Stage -in @("preprocess", "align", "train", "fast", "panorama-fast", "splat", "splat-mesh", "all")) {
    $arguments += "--force"
}
& $exe @arguments
